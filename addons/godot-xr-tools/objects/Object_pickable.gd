...
# returns true if the ball is on the floor
var on_floor = false

# where to put the ball when resetting
var reset_x = -0.423
var reset_y = 1.101
var reset_z = 1.46
...
func _on_Area_area_entered(area):
	if area.get_name() == 'paperBallArea':
		print('ball on ground')  # ball hit the floor
		
		# return the ball to the table
		#$paperBall.global_transform.origin = Vector3(-0.445,0.719,1.466)
		global_transform.origin = Vector3(-0.445,0.8,1.466)
		print('teleported')
